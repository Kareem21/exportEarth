...
export default class earth {
  ...
  async init(): Promise<void> {
    return new Promise(async (resolve) => {
      ...
      await this.createSpriteLabel() // Create labels
      this.createAnimateCircle() // Create orbiting satellites
      this.createFlyLine() // Create flight lines
  ...
  render() {
    ...
    this.flyLineArcGroup?.userData['flyLineArray']?.forEach(fly => {
      fly.rotation.z += this.options.flyLine.speed; // Adjust flight line speed
